...
const NurseSidebar: React.FC<NurseSidebarProps> = ({ activeTab, setActiveTab, isOpen, setIsOpen }) => {
  const { user, logout } = useAuth();

  const navigationItems = [
    { id: 'dashboard', label: '📊 Dashboard Overview', icon: '📊' },
    { id: 'assigned-prescriptions', label: '📄 Assigned Prescriptions', icon: '📄' },
    { id: 'drug-interactions', label: '⚠️ Drug Interaction Warnings', icon: '⚠️' },
    { id: 'mark-checked', label: '✅ Mark as Checked', icon: '✅' },
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 shadow-lg transition-all duration-300 ${isOpen ? 'w-64' : 'w-16'} h-full`}>
      <div className="p-4">
        <div className="flex items-center space-x-2">
          ...
          {isOpen && (
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Guardian X - Nurse</p>
            </div>
          )}
        </div>
      </div>
      ...
      {isOpen && user && (
        <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center mt-1">
            ...
            <span className="text-xs text-purple-600 dark:text-purple-400">Healthcare Professional</span>
          </div>
        </div>
      )}
          ...
          >
            ...
            {isOpen && (
              <div className="flex items-center justify-between w-full">
                ...
                {item.id === 'drug-interactions' && urgentCount > 0 && (
